...
function HslMenuPanelDeckListviewItemMixin:OnMouseDown()
    HearthstoneLite.contentPanel.cardViewer:Show()
    HearthstoneLite.deckViewer:Show()
    if HSL.decks and HSL.decks[self.classID] and next(HSL.decks[self.classID]) then
        for k, deck in ipairs(HSL.decks[self.classID]) do
            if deck.id == self.deckID then
                ...
                HearthstoneLite.deckViewer.classID = self.classID;
                HearthstoneLite.deckViewer.deckID = self.deckID;

                for i = 1, GetNumClasses() do
                    local className, classFile, classID = GetClassInfo(i)
                    if self.classID == classID then
                        HearthstoneLite.contentPanel.cardViewer.showClass:SetBackground_Atlas(string.format("classicon-%s", classFile:lower()))
                        HearthstoneLite.contentPanel.cardViewer.showClass.func = function()
                            HearthstoneLite.contentPanel.cardViewer:LoadCards(hsl.db.cards[classFile:lower()])
                        end
                    end
                end
...
function HslCardListviewItemMixin:OnMouseDown()
    local classID = HearthstoneLite.deckViewer.classID;
    local deckID = HearthstoneLite.deckViewer.deckID;

    if IsControlKeyDown() then
        HearthstoneLite.deckViewer:RemoveCard(self.card)
    end
end
...
function HslCardMixin:OnMouseDown()
    if IsControlKeyDown() then
        HearthstoneLite.deckViewer:AddCard(self.card)
    end
end
